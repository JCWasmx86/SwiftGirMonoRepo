...
public extension MainContextRef {
    ...
    /// Creates a new `GMainContext` structure.
    @inlinable init(flags: MainContextFlags) {
            let result = g_main_context_new_with_flags(flags.value)
        let rv = result
            ptr = UnsafeMutableRawPointer(rv)
    }
    /// Creates a new `GMainContext` structure.
    @inlinable static func newWith(flags: MainContextFlags) -> MainContextRef! {
            let result = g_main_context_new_with_flags(flags.value)
...
open class MainContext: MainContextProtocol {
    ...
    /// Creates a new `GMainContext` structure.
    @inlinable public init(flags: MainContextFlags) {
            let result = g_main_context_new_with_flags(flags.value)
        let rv = result
            ptr = UnsafeMutableRawPointer(rv)
    }
    ...
    /// Creates a new `GMainContext` structure.
    @inlinable public static func newWith(flags: MainContextFlags) -> MainContext! {
            let result = g_main_context_new_with_flags(flags.value)
