...
public extension PtrArrayRef {
    ...
    /// be `nil` terminated. A `nil` terminated pointer array has an
    /// additional `nil` pointer after the last element, beyond the
    /// current length.
    /// 
    /// `GPtrArray` created by other constructors are not automatically `nil`
    /// terminated.
    /// 
    /// Note that if the `array`'s length is zero and currently no
    /// data array is allocated, then pdata will still be `nil`.
    /// `GPtrArray` will only `nil` terminate pdata, if an actual
    /// array is allocated. It does not guarantee that an array
    /// is always allocated. In other words, if the length is zero,
    /// then pdata may either point to a `nil` terminated array of length
    /// zero or be `nil`.
    @inlinable static func newNullTerminated(null_terminated reservedSize: Int, elementFreeFunc: GDestroyNotify? = nil, nullTerminated: Bool) -> PtrArrayRef! {
            let result = g_ptr_array_new_null_terminated(guint(reservedSize), elementFreeFunc, gboolean((nullTerminated) ? 1 : 0))
        guard let rv = PtrArrayRef(gconstpointer: gconstpointer(result)) else { return nil }
        return rv
    }
...
open class PtrArray: PtrArrayProtocol {
    ...
    /// then pdata may either point to a `nil` terminated array of length
    /// zero or be `nil`.
    @inlinable public static func newNullTerminated(null_terminated reservedSize: Int, elementFreeFunc: GDestroyNotify? = nil, nullTerminated: Bool) -> PtrArray! {
            let result = g_ptr_array_new_null_terminated(guint(reservedSize), elementFreeFunc, gboolean((nullTerminated) ? 1 : 0))
        guard let rv = PtrArray(gconstpointer: gconstpointer(result)) else { return nil }
        return rv
    }
...
public extension PtrArrayProtocol {
    ...
    /// `array`. The copy will also be `nil` terminated if (and only if) the source
    /// array is.
    ...
    /// pointing to) are copied to the new `GPtrArray`.
    /// 
    /// Whether `array_to_extend` is `nil` terminated stays unchanged by this function.
    ...
    /// 
    /// Note that if the array is `nil` terminated and `free_seg` is `false`
    /// then this will always return an allocated `nil` terminated buffer.
    /// If pdata is previously `nil`, a new buffer will be allocated.
    /// 
    ...
    /// to the caller.
    /// 
    /// Note that if the array is `nil` terminated this may still return
    /// `nil` if the length of the array was zero and pdata was not yet
    /// allocated.
    /// 
    ...
    /// Gets whether the `array` was constructed as `nil`-terminated.
    /// 
    ...
    /// return `true` for normal arrays which have had a `nil` element appended to
    /// them.
    @inlinable var isNullTerminated: Bool {
        /// Gets whether the `array` was constructed as `nil`-terminated.
        /// 
        /// This will only return `true` for arrays constructed by passing `true` to the
        /// `null_terminated` argument of `g_ptr_array_new_null_terminated()`. It will not
        ...
        /// them.
        get {
            let result = g_ptr_array_is_null_terminated(ptr_array_ptr)
        let rv = ((result) != 0)
            return rv
        }
    }
