...
@inlinable public func alignedAlloc(nBlocks: Int, nBlockBytes: Int, alignment: Int) -> gpointer? {
    let result = g_aligned_alloc(gsize(nBlocks), gsize(nBlockBytes), gsize(alignment))
...
@inlinable public func alignedAlloc0(nBlocks: Int, nBlockBytes: Int, alignment: Int) -> gpointer? {
    let result = g_aligned_alloc0(gsize(nBlocks), gsize(nBlockBytes), gsize(alignment))
    guard let rv = result else { return nil }
    return rv
}
...
@inlinable public func alignedFree(mem: gpointer? = nil) {
...
@inlinable public func atomicIntCompareAndExchangeFull(atomic: UnsafeMutablePointer<gint>!, oldval: Int, newval: Int, preval: UnsafeMutablePointer<gint>!) -> Bool {
    let result = g_atomic_int_compare_and_exchange_full(atomic, gint(oldval), gint(newval), preval)
    let rv = ((result) != 0)
    return rv
}
...
/// 
/// This call acts as a full compiler and hardware memory barrier.
@inlinable public func atomicIntExchange(atomic: UnsafeMutablePointer<gint>!, newval: Int) -> Int {
    let result = g_atomic_int_exchange(atomic, gint(newval))
    let rv = Int(result)
    return rv
}
...
@inlinable public func atomicPointerCompareAndExchangeFull(atomic: UnsafeMutableRawPointer!, oldval: gpointer? = nil, newval: gpointer? = nil, preval: UnsafeMutableRawPointer!) -> Bool {
    let result = g_atomic_pointer_compare_and_exchange_full(atomic, oldval, newval, preval)
    let rv = ((result) != 0)
    return rv
}
...
/// 
/// This call acts as a full compiler and hardware memory barrier.
@inlinable public func atomicPointerExchange(atomic: UnsafeMutableRawPointer? = nil, newval: gpointer? = nil) -> gpointer? {
    let result = g_atomic_pointer_exchange(atomic, newval)
    guard let rv = result else { return nil }
    return rv
}
...
@inlinable public func datalistIDRemoveMultiple(datalist: UnsafeMutablePointer<UnsafeMutablePointer<GData>?>!, keys: UnsafeMutablePointer<GQuark>!, nKeys: Int) {
    
    g_datalist_id_remove_multiple(datalist, keys, gsize(nKeys))
...
/// The return value is cached and modifying it at runtime is not supported, as
/// it’s not thread-safe to modify environment variables at runtime.
@inlinable public func getUserStateDir() -> String! {
    let result = g_get_user_state_dir()
    guard let rv = result.map({ String(cString: $0) }) else { return nil }
    return rv
}
...
@inlinable public func hashTableNewSimilar<HashTableT: HashTableProtocol>(otherHashTable: HashTableT) -> HashTableRef! {
    let result = g_hash_table_new_similar(otherHashTable.hash_table_ptr)
...
@inlinable public func idleAddOnce(function: GSourceOnceFunc?, data: gpointer? = nil) -> Int {
    let result = g_idle_add_once(function, data)
    let rv = Int(result)
    return rv
}
...
@inlinable public func logGetDebugEnabled() -> Bool {
    let result = g_log_get_debug_enabled()
    let rv = ((result) != 0)
    return rv
}
...
@inlinable public func logSetDebug(enabled: Bool) {
    
    g_log_set_debug_enabled(gboolean((enabled) ? 1 : 0))
...
@inlinable public func timeoutAddOnce(interval: Int, function: GSourceOnceFunc?, data: gpointer? = nil) -> Int {
    let result = g_timeout_add_once(guint(interval), function, data)
    let rv = Int(result)
    return rv
}
