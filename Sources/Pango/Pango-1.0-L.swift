...
public extension LayoutRef {
    ...
    @inlinable static func deserialize<ContextT: ContextProtocol, GLibBytesT: GLib.BytesProtocol>(context: ContextT, bytes: GLibBytesT, flags: LayoutDeserializeFlags) throws -> LayoutRef! {
...
open class Layout: GLibObject.Object, LayoutProtocol {
    ...
    /// the one that was serialized, you can compare `bytes` to the
    /// result of serializing the layout again.
    @inlinable public static func deserialize<ContextT: ContextProtocol, GLibBytesT: GLib.BytesProtocol>(context: ContextT, bytes: GLibBytesT, flags: LayoutDeserializeFlags) throws -> Layout! {
            var error: UnsafeMutablePointer<GError>?
        let result = pango_layout_deserialize(context.context_ptr, bytes.bytes_ptr, flags.value, &error)
        if let error = error { throw GLibError(error) }
...
public extension LanguageRef {
    ...
    @inlinable static func getPreferred() -> UnsafeMutablePointer<UnsafeMutablePointer<PangoLanguage>?>! {
...
open class Language: LanguageProtocol {
    ...
    /// you should first try the default language, followed by the
    /// languages returned by this function.
    @inlinable public static func getPreferred() -> UnsafeMutablePointer<UnsafeMutablePointer<PangoLanguage>?>! {
            let result = pango_language_get_preferred()
        guard let rv = result else { return nil }
            return rv
    }
