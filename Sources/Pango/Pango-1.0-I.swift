...
public extension ItemProtocol {
    ...
    @inlinable func copy() -> ItemRef! {
        let result = pango_item_copy(item_ptr)
        guard let rv = ItemRef(gconstpointer: gconstpointer(result)) else { return nil }
        return rv
    }
    ...
    @inlinable func split(splitIndex: Int, splitOffset: Int) -> ItemRef! {
        let result = pango_item_split(item_ptr, gint(splitIndex), gint(splitOffset))
        guard let rv = ItemRef(gconstpointer: gconstpointer(result)) else { return nil }
        return rv
    }
