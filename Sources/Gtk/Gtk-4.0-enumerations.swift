...
public typealias ContentFit = GtkContentFit


public extension ContentFit {
    /// Cast constructor, converting any binary integer to a
    /// `ContentFit`.
    /// - Parameter raw: The raw integer value to initialise the enum from
    @inlinable init!<I: BinaryInteger>(_ raw: I) {
        func castToContentFitInt<I: BinaryInteger, J: BinaryInteger>(_ param: I) -> J { J(param) }
        self.init(rawValue: castToContentFitInt(raw))
    }
    ...
    static let fill = GTK_CONTENT_FIT_FILL // 0
    ...
    ///   from the allocation aspect ratio.
    static let contain = GTK_CONTENT_FIT_CONTAIN // 1
...
public typealias InscriptionOverflow = GtkInscriptionOverflow


public extension InscriptionOverflow {
    /// Cast constructor, converting any binary integer to a
    /// `InscriptionOverflow`.
    /// - Parameter raw: The raw integer value to initialise the enum from
    @inlinable init!<I: BinaryInteger>(_ raw: I) {
        func castToInscriptionOverflowInt<I: BinaryInteger, J: BinaryInteger>(_ param: I) -> J { J(param) }
        self.init(rawValue: castToInscriptionOverflowInt(raw))
    }
    /// Clip the remaining text
    static let clip = GTK_INSCRIPTION_OVERFLOW_CLIP // 0

    /// Omit characters at the start of the text
    static let ellipsizeStart = GTK_INSCRIPTION_OVERFLOW_ELLIPSIZE_START // 1

    /// Omit characters at the middle of the text
    static let ellipsizeMiddle = GTK_INSCRIPTION_OVERFLOW_ELLIPSIZE_MIDDLE // 2

    /// Omit characters at the end of the text
    static let ellipsizeEnd = GTK_INSCRIPTION_OVERFLOW_ELLIPSIZE_END // 3
...
public typealias NaturalWrapMode = GtkNaturalWrapMode


public extension NaturalWrapMode {
    /// Cast constructor, converting any binary integer to a
    /// `NaturalWrapMode`.
    /// - Parameter raw: The raw integer value to initialise the enum from
    @inlinable init!<I: BinaryInteger>(_ raw: I) {
        func castToNaturalWrapModeInt<I: BinaryInteger, J: BinaryInteger>(_ param: I) -> J { J(param) }
        self.init(rawValue: castToNaturalWrapModeInt(raw))
    }
    /// Inherit the minimum size request.
    ///   In particular, this should be used with `PANGO_WRAP_CHAR`.
    static let inherit = GTK_NATURAL_WRAP_INHERIT // 0
...
public typealias SymbolicColor = GtkSymbolicColor


public extension SymbolicColor {
    /// Cast constructor, converting any binary integer to a
    /// `SymbolicColor`.
    /// - Parameter raw: The raw integer value to initialise the enum from
    @inlinable init!<I: BinaryInteger>(_ raw: I) {
        func castToSymbolicColorInt<I: BinaryInteger, J: BinaryInteger>(_ param: I) -> J { J(param) }
        self.init(rawValue: castToSymbolicColorInt(raw))
    }
    /// The default foreground color
    static let foreground = GTK_SYMBOLIC_COLOR_FOREGROUND // 0

    /// Indication color for errors
    static let error = GTK_SYMBOLIC_COLOR_ERROR // 1

    /// Indication color for warnings
    static let warning = GTK_SYMBOLIC_COLOR_WARNING // 2

    /// Indication color for success
    static let success = GTK_SYMBOLIC_COLOR_SUCCESS // 3
